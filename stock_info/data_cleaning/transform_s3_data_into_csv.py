...
def parse_data_to_df(stock_info):
    ...
    try:
        json_file = df_temp.to_json(orient='records', force_ascii=False)
        put_object_to_s3(date_of_str, json_file, 'temp')
        logger.info('succeed')
    except:
        logger.info('failed')
    # rename the columns
    ...
    # transform data type
    try:
        df_stock['trade_volume_shared'] = df_stock['trade_volume_shared'].astype(
            int)
        ...
        df_stock['last_best_bid_price'] = df_stock['last_best_bid_price'].astype(
            float)
        df_stock['last_beast_bid_volume'] = df_stock['last_best_bid_volume'].astype(
            int)
        df_stock['last_best_ask_price'] = df_stock['last_best_ask_price'].astype(
            float)
        df_stock['last_beast_ask_volume'] = df_stock['last_best_ask_volume'].astype(
            int)
    except:
        pass
...
def get_bucket_keys_from_s3():
    ...
    for obj in object_response['Contents']:
        ...
        if ('cleaned_data' not in obj['Key'] and len(obj['Key']) > 20):
            print(obj['Key'])
            files.append({
                "Key": obj['Key'],
...
def transform_data_into_df(event, context):
    ...
    for date_info in files:
        ...
        res = client.get_object(
            Bucket='serverless-stocks', Key=date_info['Key'])
    ...
    try:
        ...
        put_object_to_s3(date_of_str, json_file, 'success')
        logger.info('succeed')
    except:
        logger.info('failed')
